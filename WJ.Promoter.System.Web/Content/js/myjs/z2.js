$(function () {
    ...
    function gridList() {
        ...
        $gridList.dataGrid({
            ...
            colModel: [
                ...
                { label: '3级代理贡献收益应返', name: 'Level3ReadyRepay', align: 'center', formatter: $.toMoney },
                { label: '3级代理收益已返', name: 'Level3AlreadyRepay', align: 'center', formatter: $.toMoney },
                {
    ...
    function thridHandler(vv) {
        ...
        var ldt = parent.document.getElementById('lithrdetail');
        var $ldt = $(ldt);
        var home = parent.document.getElementById('home');
        var $home = $(home);
        var sea = parent.document.getElementById('a_thrdetail');
        //$(sea).tab('show');

        var secdetail = parent.document.querySelector('#thrdetail');
        ...
        if (vv != f || hh != aa.UserId) {
            ...
            var fsecdetail = parent.document.querySelector('#fthrdetail');
            //var secdetail = parent.document.querySelector('#thrdetail');
            ...
            if ($fsecdetail.length > 0)
                $fsecdetail.remove();
            var str1 = '<iframe  class="NFine_iframe"   id="fthrdetail"  frameborder="0"  rowid="' + vv + '"  gid="' + a + '"  width="' + h + '"  height="' + g + '"  src="' + "./ThridAgencyDataAnalysisDetail.html?a=" + a + '&c=0&b=' + aa.UserId + '" ></iframe>';
