...
$(function ($) {
    ...
    $('#sidebar-nav,#nav-col-submenu').on('click', '.dropdown-toggle', function (e) {
        //if (e.target.id == 'grzx' || e.target.parentNode.id) {
        //    e.preventDefault();
...
function GetLoadNav() {
    ...
    $.each(data, function (i) {
        ...
        if (row.Pid == "0") {
            _html += '<li>';
            _html += '<a id="' + row.Id + '" data-id="' + row.Id + '" href="' + row.ModuleValue + '" class="dropdown-toggle"><i class="' + row.Icon + '"></i><span>' + row.ModuleName + '</span><i class="fa fa-angle-right drop-icon"></i></a>';
            ...
            if (childNodes.length > 0) {
                ...
                $.each(childNodes, function (i) {
                    ...
                    if (subrow.Id == 7)
                        mark = 1;
                    _html += '<li>';
                    _html += '<a class="menuItem"   id="' + subrow.Id + '" data-id="' + subrow.Id + '" href="' + subrow.ModuleValue + '" data-index="' + i + '">' + subrow.ModuleName + '</a>';
                    _html += '</li>';
                });
                _html += '</ul>';
            }
    ...
    if (mark == 0) {
        $('#frist').attr('data-id', './HomePages/index.html');
        $('#content-main .NFine_iframe').attr('src', './HomePages/index.html').attr('data-id', './HomePages/index.html');
    }
    $("#sidebar-nav ul").prepend(_html);
}
$(function ($) {
    $("#drrz").on('click', function () {
        $.modalOpenGrid({
            id: "Form",
            title: '登入日志',
            url: encodeURI("./HomePages/Form_UserLogs.html"),
            width: "600px",
            height: "600px",
    ...
    $("#loginout").on('click', function () {
        $.get('/DataCenter/BaseData.aspx?method=LoginOut', function () {
            top.location = '../Pages/login.html'
        })
    })

    $.get('/DataCenter/BaseData.aspx?method=CurrentUser', function (data) {
        var d = $.parseJSON(data);
        if (d && d.length > 0) {
            $('#u_name').text(d[0].UserName);
            $('#u_rname').text(d[0].RoleName);
            ...
            $('#editPass').on('click', function () {
                ...
                $.modalOpen({
                    id: "Form",
                    title: "修改密码",
                    url: encodeURI("./HomePages/Form_EditPass.html?a=" + a),
                    width: "500px",
                    height: "400px",
                    callBack: function (iframeId) {
                        top.frames[iframeId].submitForm();
